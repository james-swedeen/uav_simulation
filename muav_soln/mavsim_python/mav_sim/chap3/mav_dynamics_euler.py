...
def derivatives_euler(state: types.DynamicStateEuler, forces_moments: types.ForceMoment) -> types.DynamicStateEuler:
    ...
    # position kinematics (Equation 3.14)
    pos_dot = Euler2Rotation(phi=st.phi, theta=st.theta, psi=st.psi) @ state[IND_EULER.VEL]
    north_dot = pos_dot.item(0)
    east_dot = pos_dot.item(1)
    down_dot = pos_dot.item(2)

    # translational dynamics (Equation 3.15)
    u_dot = st.r*st.v - st.q*st.w + fm.fx/MAV.mass
    v_dot = st.p*st.w - st.r*st.u + fm.fy/MAV.mass
    w_dot = st.q*st.u - st.p*st.v + fm.fz/MAV.mass

    # rotational kinematics (Equation 3.16)
    body_to_euler = np.array([[1., np.sin(st.phi)*np.tan(st.theta), np.cos(st.phi)*np.tan(st.theta)],
                              [0., np.cos(st.phi),                  -np.sin(st.phi)],
                              [0., np.sin(st.phi)/np.cos(st.theta), np.cos(st.phi)/np.cos(st.theta)]])
    att_dot = body_to_euler@state[IND_EULER.ANG_VEL]
    phi_dot = att_dot.item(0)
    theta_dot = att_dot.item(1)
    psi_dot = att_dot.item(2)

    # rotatonal dynamics (Equation 3.17)
    ...
    x_dot[IND_EULER.NORTH] = north_dot
    x_dot[IND_EULER.EAST] = east_dot
    x_dot[IND_EULER.DOWN] = down_dot
    x_dot[IND_EULER.U] = u_dot
    x_dot[IND_EULER.V] = v_dot
    x_dot[IND_EULER.W] = w_dot
    x_dot[IND_EULER.PHI] = phi_dot
    x_dot[IND_EULER.THETA] = theta_dot
    x_dot[IND_EULER.PSI] = psi_dot
