...
def gyro(p: float, q: float, r: float, noise_scale: float = 1.,
    ...
    gyro_x = p + gyro_x_bias \
                + noise_scale * np.random.normal(0., gyro_sigma)
    gyro_y = q + gyro_y_bias \
                + noise_scale * np.random.normal(0., gyro_sigma)
    gyro_z = r + gyro_z_bias\
                + noise_scale * np.random.normal(0., gyro_sigma)
...
def pressure(down: float, Va: float, noise_scale: float = 1.,
    ...
    abs_pressure = -MAV.rho * MAV.gravity * down + abs_pres_bias \
                        + noise_scale * np.random.normal(0., abs_pres_sigma)

    # Implementation of equation (7.10)
    diff_pressure = 0.5 * MAV.rho * Va**2 + diff_pres_bias \
...
def magnetometer(quat_b_to_i: types.Quaternion, noise_scale: float = 1.,
    ...
    # Add zero mean white noise using a standard deviation of SENSOR.mag_sigma
...
def gps_error_trans_update(nu: GpsTransient, noise_scale: float = 1.,
    ...
    nu_n = np.exp(-gps_k*ts_gps)*nu.n \
                            + noise_scale * ts_gps * np.random.normal(0., gps_n_sigma)
    nu_e = np.exp(-gps_k*ts_gps)*nu.e \
                        + noise_scale * ts_gps * np.random.normal(0., gps_e_sigma)
    nu_h = np.exp(-gps_k*ts_gps)*nu.h \
                        + noise_scale * ts_gps * np.random.normal(0., gps_h_sigma)
