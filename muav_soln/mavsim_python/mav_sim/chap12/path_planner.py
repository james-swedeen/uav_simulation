...
from mav_sim.chap12.rrt_dubins import RRTDubins
...
class PathPlanner:
    ...
    def update(self, world_map: MsgWorldMap, state: MsgState, \
        ...
        if planner_type == PlannerType.simple_fillet:
            ...
            # Adjust the start point to be in front of UAV for finding a feasible path
            start_pose_leading = start_pose.copy()
            c_chi = np.cos(state.chi)
            s_chi = np.sin(state.chi)
                # A value of 1.5 gives the planner a minimum radius plus some additional room for planning
            start_pose_leading[0:2] = start_pose[0:2] + 1.5*self.radius*np.array([[c_chi], [s_chi]])
            h_start_adj = height_above_ground(world_map=world_map, point=start_pose_leading)
            if h_start_adj <= 0.:
                raise ValueError("Adjusted start pose must be above ground")

            # Plan from the adjust start node
            self.waypoints = self.rrt_straight_line.update(start_pose_leading[0:3], end_pose,
                                                           desired_airspeed, world_map)

            # Add the actual start node onto the plan
            self.waypoints.prepend(ned=start_pose[0:3], course=state.chi, airspeed=desired_airspeed)


        elif planner_type == PlannerType.rrt_dubins:
            self.waypoints = self.rrt_dubins.update(start_pose, end_pose,
                                                    desired_airspeed, world_map, self.radius)
        else:
