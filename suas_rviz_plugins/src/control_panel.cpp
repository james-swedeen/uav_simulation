  ...
  : Node("control_panel_node")
{
  ...
  this->state_listener_toggle_client =
    this->create_client<std_srvs::srv::Empty>("state_listener_toggle_topic");
  this->sense_auto_dyn_reset_client =
    this->create_client<std_srvs::srv::Empty>("reset_autopilot");
  this->clock_client =
...
void ControlPanelNode::startStopDynamics() {
  ...
  if(state_listener_toggle_client->wait_for_service(1s)) {
    auto request = std::make_shared<std_srvs::srv::Empty::Request>();
    auto result = state_listener_toggle_client->async_send_request(request);
    RCLCPP_DEBUG(this->get_logger(), "Toggle service sent to StateListener");
  }
}
...
void ControlPanelNode::resetState() {
  ...
  if(sense_auto_dyn_reset_client->wait_for_service(1s)) {
    auto request = std::make_shared<std_srvs::srv::Empty::Request>();
    auto result = sense_auto_dyn_reset_client->async_send_request(request);
    RCLCPP_DEBUG(this->get_logger(), "Reset service sent to Autopilot");
  }
}
