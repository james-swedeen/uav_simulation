...
class PathManagerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._pub_path_ready = self.create_publisher(Empty, "path_ready", 1)     # Notification that path is ready to follow
        self._pub_path_end = self.create_publisher(Empty, "end_path_reached", 1) # Notification that end of path reached
    ...
    def _waypoints_callback(self, msg: UavWaypoints) -> None:
        ...
        try:
            ...
            self._ended_run = False
            self._pub_path_ready.publish(Empty())
            # Send ready message to sim manager
    ...
    def _main_loop(self) -> None:
        ...
        # Check to see if reached end of path
        if self._ended_run:
            self._pub_path_end.publish(Empty())

        if self._path_manager.manager_requests_waypoints() and not self._ended_run:
            self._status.level = DiagnosticStatus.OK
            self._status.message = "Finished path"

            # Publish message indicating path completion
            self._ended_run = True
            self._pub_path_end.publish(Empty())
